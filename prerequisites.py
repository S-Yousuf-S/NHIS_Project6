...
================================================================================
 ...
 Description  : Shared, utility module imported by all 4 notebooks:
...
def setup_logging(notebook_name="notebook", log_dir="Logs", level=logging.INFO):
    """
    Configures a logger that writes to both console and a date-stamped log
    file under Logs/, prefixed with "YS_" for ownership. Filename
    uses DATE only (not time), and the file is opened in append mode - so
    every run on the same day accumulates into one file instead of creating
    a new file per run. Call once per notebook session.
    """
...
# ------------------------------------------------------------------------------
# Semantic log wrappers - mirror the wording used in the HTML message boxes
# (success_msg/warning_msg/info_msg) so log text and on-screen text stay
# consistent. Python's logging module has no built-in SUCCESS level, so it's
# implemented as an INFO-level call with an explicit "SUCCESS:" prefix.
# ------------------------------------------------------------------------------
...
def set_global_visualization_settings(palette="viridis"):
    ...
    success_box(f"Global visualization settings are ready")
